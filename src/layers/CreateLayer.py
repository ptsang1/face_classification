    ...
    def __init__(self):
        self.layers = {
            ...
            'resnet_basic_block': lambda input_shape, **kwargs: (
                nn.Sequential(nn.Conv2d(input_shape[0], input_shape[0], bias=False, kernel_size=3, padding=1),
                              nn.BatchNorm2d(input_shape[0]),
                              nn.ReLU(),
                              nn.Conv2d(input_shape[0], input_shape[0], bias=False, kernel_size=3, padding=1),
                              nn.BatchNorm2d(input_shape[0])), input_shape)
        }
